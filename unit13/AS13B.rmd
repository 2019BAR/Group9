...
TR = subset(D, split == TRUE)
TS = subset(D, split == FALSE)
glm1 = glm(PoorCare ~ OfficeVisits + Narcotics + InpatientDays + ERVisits + DaysSinceLastERVisit + Pain + TotalVisits + ProviderCount + MedicalClaims +　ClaimLines + AcuteDrugGapSmall , TR, family=binomial)
summary(glm1)
```
...
##Ans. 有比較準，因原本Test的準確性由0.812變為0.844，可知其準確性提高。
&emsp; 【F】 為什麼它比較準(或比較不準)呢？<br><br>
##Ans. 因加入的變數變多，解釋力提高，更易找到和此適配的模型，所以以Test的data來說準確性上升的機率變大，會較準。
</p class="qiz">
