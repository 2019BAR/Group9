...
```{r}
#0.10412
table(D$Arrest, format(ts, '%Y')) %>% prop.table(2)
```
...
Top5 = table(D$LocationDescription) %>% sort %>% tail(6)
Top5
```
...
```{r}
#Gas station
topName = names(table(D$LocationDescription) %>% sort %>% tail(6))[-4]
topName
TOP5 = subset(D, LocationDescription %in% topName)
...
```{r}
#星期六
ts[D$LocationDescription == "GAS STATION"] %>% format("%w") %>% table %>% sort
```

【4.5】 On which day of the week do the fewest motor vehicle thefts in residential driveways happen?
```{r}
#星期六
ts[D$LocationDescription == "DRIVEWAY - RESIDENTIAL"] %>% format("%w") %>% table %>% sort
```
