...
```{r}
###setwd("C:/Users/Alice/Desktop/Group9/unit03")
library(readr)
CPS = read_csv( "data/CPSData.csv")  ###131302人
```
...
```{r}
which.min(table(CPS$State)) ### New Mexico
```

Which state has the largest number of interviewees?
```{r}
which.max(table(CPS$State)) ### California
```

__§ 1.4__ What proportion of interviewees are citizens of the United States?
```{r}
table(CPS$Citizenship)
(116639 + 7073) / (7590 + 116639 + 7073) ### 0.9422
```
# 116639
...
```{r}
CPS[,c(2,3,4,6,7,8,9,12,13)]  <- lapply(CPS[,c(2,3,4,6,7,8,9,12,13)], factor)
...
```{r}
sort(tapply(is.na(CPS$MetroAreaCode), CPS$State, mean)) %>% sort(decreasing = TRUE)
### Montana
```
...
```{r}
Metro <- read_csv("data/MetroAreaCodes.csv")
str(Metro) ### 271 obs
```

How many observations (codes for countries) are there in CountryMap?
```{r}
Country <- read_csv("data/CountryCodes.csv")
str(Country) ### 149 obs
```

__§ 3.2__ What is the name of the variable that was added to the data frame by the merge() operation?
```{r}
CPS = merge(CPS, Metro, by.x="MetroAreaCode", by.y="Code", all.x=TRUE) ### MetroArea
```
...
```{r}
table(CPS$MetroArea) %>% sort(decreasing = TRUE) %>% head(8)
...
```{r}
tapply(CPS$Hispanic, CPS$MetroArea, mean) %>% sort(decreasing = TRUE) %>% head() ### Laredo, TX (0.9663)
```
...
```{r}
tapply(CPS$Race == "Asian", CPS$MetroArea, mean) %>% sort(decreasing = TRUE) %>% head(10) ### 4
```
...
```{r}
tapply(CPS$Education == "No high school diploma", CPS$MetroArea, mean, na.rm = TRUE) %>% sort() %>% head(10) ### Iowa City, IA(0.02913)
```
...
How many interviewees have a missing value for the new metropolitan area variable?
```{r}
CPS$Country <- as.factor(CPS$Country)
summary(CPS) ### Country 176 NA's
```

__§ 4.2__ Among all interviewees born outside of North America, which country was the most common place of birth?
```{r}
table(CPS$Country) %>% sort(decreasing = TRUE) %>% head(5) ### Philippines 839
```
...
```{r}
tapply(CPS$Country == "India", CPS$MetroArea, sum, na.rm=TRUE) %>% sort(decreasing = TRUE) %>% head() ### New York-Northern New Jersey-Long Island, NY-NJ-PA (96)
```

In Brazil?
```{r}
tapply(CPS$Country == "Brazil", CPS$MetroArea, sum, na.rm=TRUE) %>% sort(decreasing = TRUE) %>% head()
###  Boston-Cambridge-Quincy, MA-NH (18)
```

In Somalia?
```{r}
tapply(CPS$Country == "Somalia", CPS$MetroArea, sum, na.rm=TRUE) %>% sort(decreasing = TRUE) %>% head()
